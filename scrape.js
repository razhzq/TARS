...
async function scrapeTweets(page) {
  ...
  // set target time to 4 hours ago
...
async function removeDuplicate(tweetsArray) {
    ...
    for(let i=0; i< tweetsArray.length; i++) {
      if(tweetsDb.some((tweet) => tweet.tweetString === tweetsArray[i].tweetString)) continue;
...
function extractToken(newTweetsArray) {
  ...
  for(let i=0; i < newTweetsArray.length; i++) {
      ...
      if(matches.length > 0) {
        newTweetsArray[i].signals = matches.map((match) => match[1].toLowerCase());
...
async function saveTweetsToDb(tokenMatchedTweets) {
     for(let i=0; i< tokenMatchedTweets.length; i++) {
     
         const tweetSignals = tokenMatchedTweets[i].signals;
         const accountId = await Token.getTokenId()
         const accountTier = await Account.getAccountTier(tokenMatchedTweets[i].tweetWho); 
         for(let j =0; j < tweetSignals.length; j++) {
           ...
           await Tweet.saveNewTweet(tokenMatchedTweets[i].tweetString, tokenMatchedTweets[i].tweetWho, tokenMatchedTweets[i].tweetTime, tweetSignals[j]);
